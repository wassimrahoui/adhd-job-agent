...
Adzuna's description snippet, email content, and live web pages during automation are **external, untrusted input** by default, everywhere in this system. Concretely:
...
- **They cannot exfiltrate data.** The LLM call is a local, outbound-only-to-`localhost`/local-network Ollama call with a bounded context (the specific job + relevant CV/profile fields); it has no network tool, no file tool, and no ability to include arbitrary other users' or system data.
- **Automation cannot be redirected by page content.** As covered in `03-job-sources-and-browser-automation.md`, the Playwright controller executes a fixed, code-defined action plan; it has no natural-language instruction-following pathway, so text on a page (e.g. a fake "click here to auto-apply" banner) cannot cause it to deviate from its plan. Page-identity verification before every action further prevents acting on the wrong page even if navigation was somehow redirected.
...
- **Adzuna itself is a trusted, authenticated API call**, not untrusted content — but the *job text it returns* is still treated as untrusted external text for prompt-construction purposes, same as any posting would be.
...
flowchart LR
    ...
    subgraph Blocked["Why it can't work"]
        ...
        B2["Automation follows a fixed code plan,\nnot page/LLM instructions"]
...
| Compromised/rogue browser page during automation | Page-identity verification before every action; fixed action plans; independent state read-back |
| Cross-user data access | JWT-scoped queries + RLS defense-in-depth |
| Credential/secret leakage (DB, mailbox, Adzuna) | Server-side-only secrets, never logged, scanned out of evidence/audit payloads |
