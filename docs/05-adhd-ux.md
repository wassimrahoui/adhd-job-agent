...
2. **Job Review** — one job at a time. Title, company, location/salary/work-mode at a glance, deterministic factor breakdown, AI explanation with Verified/Inferred/Unknown labels, and three big actions: Approve, Save, Dismiss. A secondary "view original posting" link for anyone who wants to double-check (rarely needed, precisely because the system already shows its evidence).
...
4. **Application Prep** — shows staged materials and, where automation filled a form, the filled form for review, with one clear "Submit" action gated behind explicit confirmation, and an equally easy "Not now" / "I'll apply manually" path.
